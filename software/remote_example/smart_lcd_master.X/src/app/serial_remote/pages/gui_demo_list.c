 ...
 * @brief flexible list (through macro) demo (serialized, master side)
 ...
 * local functions
 */
static void Gui_DemoListRead(void);
static void Gui_DemoListHandler(void);
...
void Gui_DemoList(void) {
  ...
  R_GUI_M_ListAdd(&rec, true, true, false, E_PUSHED_TO_RELEASED, SIG_LIST);

  /*add categories*/
  R_GUI_M_ListAddCategoryToList(70, "id");
  R_GUI_M_ListAddCategoryToList(220, "name");

  /*add items*/
  while(ii <= G_IMG_USER_BLUE2) {
    R_GUI_M_ListAddItemToList(ii);
    sprintf(str, "%03d", ii);
    R_GUI_M_ListAddFieldToLastItem(str);
  ...
  /*done; go to handler*/
  R_GUI_SetTask(Gui_DemoListRead);
}


/**
 * @function Gui_DemoListRead
...
static void Gui_DemoListRead(void) {
  ...
  if(IsTimerElapsed(tmRefresh)) {
    ...
    R_GUI_SetTask(Gui_DemoListHandler);
  }
}
...
static void Gui_DemoListHandler(void) {
  ...
  //uint16_t selectedItem = RxGetU16();
