...
#define CS_SETOUT() TRISEbits.TRISE8 = 0
#define CS_LOW()  _LATE8 = 0	/* MMC CS = L */
#define CS_HIGH() _LATE8 = 1	/* MMC CS = H */
...
#define SOCKPORT	PORTE		/* Socket contact port */
#define SOCKWP	0 // disable write protect (1<<10) -> micro SD, no WP
#define SOCKINS	(1<<9) // Pretend card is always inserted (1<<11)	/* Card detect switch (RB11) */
...
)
{
  ...
  TicksSetExternalCallback(disk_timerproc);
